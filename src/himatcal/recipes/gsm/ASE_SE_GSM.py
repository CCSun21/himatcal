...
from pyGSM.growing_string_methods import (
    DE_GSM,
    SE_GSM,  # type: ignore  # type: ignore
)
...
from pyGSM.optimizers.lbfgs import lbfgss
...
from pyGSM.utilities.cli_utils import (
    get_driving_coord_prim,  # type: ignore  # type: ignore
)
...
# class ASE_SE_GSM:
#     def __init__(self, atoms, driving_coords, calculator, cleanup_scratch=False):
#         self.atoms = atoms
...
#     def build_lot(self):
#         self.lot = ASELoT.from_options(
#             self.calculator, geom=[[x.symbol, *x.position] for x in self.atoms], ID=0
#         )

#     def build_pes(self):
#         # PES
#         self.pes_obj = PES.from_options(lot=self.lot, ad_idx=0, multiplicity=1)


def minimal_wrapper_se_gsm(
    atoms: Atoms,
    calculator,
    driving_coords,
):
    ...
    lot = ASELoT.from_options(
        calculator, geom=[[x.symbol, *x.position] for x in atoms], ID=ID
    )
    ...
    elements = [element_table.from_symbol(sym) for sym in atoms.get_chemical_symbols()]

    topology = Topology.build_topology(xyz=atoms.get_positions(), atoms=elements)
    ...
    for prim in driving_coord_prims:
        if type(prim) is Distance:
            ...
            if (
                bond not in topology.edges
                and (bond[1], bond[0]) not in topology.edges()
            ):
