...
from himatcal.utils.gsm._base import Connection, connection_limits


class MolConnectionError(Exception):
    """
    ...
    def is_subset(self, other):
        ...
        for idxs in self._break_idxs:
            if idxs not in other._break_idxs:
                return False
        return all(idxs in other._form_idxs for idxs in self._form_idxs)
...
def generate_driving_coords(
    ...
    all_possible_connections = [
        Connection(atom1, atom2)
        for i, atom1 in enumerate(atoms)
        for atom2 in atoms[(i + 1) :]
        if not atom1.frozen and not atom2.frozen
    ]
    all_potential_new_connections = [
        connection
    ...
    for nbreak in range(minbreak, maxbreak + 1):
        for nform in range(minform, maxform + 1):
            if nbreak + nform < minchange:
                continue
            if nbreak + nform > maxchange:
                continue
            if not single_change and (nbreak + nform == 1):
                continue

            # Generate all possible combinations of connection changes
            potential_remove_connections_iter = itertools.combinations(
                connections, nbreak
            )
            potential_new_connections_iter = itertools.combinations(
                all_potential_new_connections, nform
            )
            potential_connection_changes = itertools.product(
                potential_remove_connections_iter, potential_new_connections_iter
            )

            for (
                connections_to_break,
                connections_to_form,
            ) in potential_connection_changes:
                try:
                    change_connections(
                        mol,
                        connections_to_break,
                        connections_to_form,
                        test_validity=check_limits,
                    )
                except MolConnectionError:
                    continue
                else:
                    break_idxs = [
                        (c.atom1.idx, c.atom2.idx) for c in connections_to_break
                    ]
                    form_idxs = [
                        (c.atom1.idx, c.atom2.idx) for c in connections_to_form
                    ]
                    driving_coords = DrivingCoords(
                        break_idxs=break_idxs, form_idxs=form_idxs
                    )
...
def change_connections(
    mol, connections_to_break, connections_to_form, test_validity=True
):
    ...
    if test_validity:
        ...
        for connection in connections_to_break:
            if not test_connection_validity(connection):
                raise MolConnectionError(
                    f"Breaking {connection} resulted in violation of connection limits"
                )
        for connection in connections_to_form:
            if not test_connection_validity(connection):
                raise MolConnectionError(
                    f"Forming {connection} resulted in violation of connection limits"
                )
...
def test_connection_validity(connection):
    ...
    return (
        len(atom1.connections) >= atom1_ll
        and len(atom1.connections) <= atom1_ul
        and len(atom2.connections) >= atom2_ll
        and len(atom2.connections) <= atom2_ul
    )
